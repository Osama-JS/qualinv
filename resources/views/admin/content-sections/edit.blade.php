...
@section('title', __('admin.edit_content_section'))

@section('content')
<div class="container px-6 mx-auto grid">
    <div class="flex justify-between items-center">
        <h2 class="my-6 text-2xl font-semibold text-gray-700 dark:text-gray-200">
            {{ __('admin.edit_content_section') }}
        </h2>
        <div class="flex space-x-2">
            <a href="{{ route('admin.content-sections.show', $contentSection) }}" class="flex items-center justify-between px-4 py-2 text-sm font-medium leading-5 text-blue-700 transition-colors duration-150 bg-blue-100 border border-blue-300 rounded-lg active:bg-blue-200 hover:bg-blue-200 focus:outline-none focus:shadow-outline-blue">
    ...
    <div class="bg-white dark:bg-gray-800 shadow-md rounded-lg border border-gray-200 dark:border-gray-700">
        <div class="p-6 border-b border-gray-200 dark:border-gray-700">
            <h3 class="text-lg font-semibold text-gray-800 dark:text-white">
                {{ __('admin.edit_content_section_details') }}
            </h3>
        </div>

        <form method="POST" action="{{ route('admin.content-sections.update', $contentSection) }}" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    ...
                    <input type="text" name="title_ar" id="title_ar" value="{{ old('title_ar', $contentSection->title_ar) }}"
                ...
                <div>
                    ...
                    <input type="text" name="title_en" id="title_en" value="{{ old('title_en', $contentSection->title_en) }}"
                           class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-gray-300"
                           required>
                    @error('title_en')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
                ...
                <div>
                    ...
                    <select name="page_location" id="page_location"
                        ...
                        @foreach(\App\Models\ContentSection::getPageLocations() as $value => $label)
                            <option value="{{ $value }}" {{ old('page_location', $contentSection->page_location) == $value ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                    @error('page_location')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                ...
                <div>
                    ...
                    <input type="number" name="display_order" id="display_order" value="{{ old('display_order', $contentSection->display_order) }}" min="0"
                           class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-gray-300"
                           required>
                    @error('display_order')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                ...
                <div>
                    ...
                    <div class="flex items-center">
                        <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $contentSection->is_active) ? 'checked' : '' }}
            ...
            <div class="mb-6">
                ...
                <textarea name="content_ar" id="content_ar" rows="10"
                          class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-gray-300"
                          required>{{ old('content_ar', $contentSection->content_ar) }}</textarea>
                @error('content_ar')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-6">
                ...
                <textarea name="content_en" id="content_en" rows="10"
                          class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-gray-300"
                          required>{{ old('content_en', $contentSection->content_en) }}</textarea>
                @error('content_en')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
