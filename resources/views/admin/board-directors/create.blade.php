...
@section('page-title', __('admin.add_board_director'))

@section('content')
<div class="mb-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-bold text-gray-900 dark:text-gray-100">{{ __('admin.add_board_director') }}</h1>
            <p class="text-gray-600 dark:text-gray-400">{{ __('admin.add_new_board_director_description') }}</p>
        </div>
...
<form action="{{ route('admin.board-directors.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
    @csrf
        ...
        <div class="p-6 space-y-6">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="position_en" id="position_en" value="{{ old('position_en') }}"
                           class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-gray-100"
                           required>
                    @error('position_en')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                ...
                <div>
                    ...
                    <input type="text" name="position_ar" id="position_ar" value="{{ old('position_ar') }}"
                           class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-gray-100"
                           dir="rtl" required>
                    @error('position_ar')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <textarea name="bio_en" id="bio_en" rows="6"
                              class="tinymce-editor w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-gray-100">{{ old('bio_en') }}</textarea>
                    @error('bio_en')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
    ...
    <div class="flex justify-end space-x-4">
        ...
        <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-6 rounded-lg transition-colors">
            ...
            {{ __('admin.create_board_director') }}
        </button>
    </div>
</form>
@endsection
